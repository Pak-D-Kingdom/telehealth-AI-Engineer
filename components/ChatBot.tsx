                ...
                >
                  ...
                  >
                    ...
                    {message.isStreaming && !message.text ? (
                      ...
                      <div className="prose-sm max-w-none text-xs leading-relaxed text-gray-800 sm:text-sm">
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            table: ({ children }) => (
                              <div className="my-2.5 overflow-x-auto rounded-xl border border-gray-200 shadow-xs">
                                <table className="min-w-full divide-y divide-gray-200 text-left text-xs">{children}</table>
                              </div>
                            ),
                            thead: ({ children }) => <thead className="bg-[#0D5C46]/10 text-[#0D5C46] font-bold">{children}</thead>,
                            ...
                            th: ({ children }) => <th className="px-3 py-2 text-[11px] font-semibold tracking-wider text-[#0D5C46]">{children}</th>,
                            td: ({ children }) => <td className="px-3 py-2 text-gray-700">{children}</td>,
                            code: ({ children }) => <code className="rounded bg-gray-100 px-1.5 py-0.5 text-[11px] font-mono text-gray-800">{children}</code>,
                          }}
                        >
                          {message.text.replace(/<SBAR_READY>[\s\S]*/, "")}
                        </ReactMarkdown>
                        {message.isStreaming && (
                          <span className="ml-0.5 inline-block h-3.5 w-0.5 animate-pulse bg-[#E07A5F] align-middle" />
                        )}
                      </div>
                    )}
